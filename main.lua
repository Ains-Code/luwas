    ...
    if petFolder then
        ...
        if stockValues then
            ...
            if petsNode then
                ...
                warn("[GAG2] Available folders in StockValues:")
                for _, child in ipairs(stockValues:GetChildren()) do
...
-- Special event weather that deserves bigger alerts
local SPECIAL_WEATHER = {
    ["Midas"]      = { emoji = "✨", color = 0xFFD700 },      -- Gold
    ["Rainbow"]    = { emoji = "🌈", color = 0xFF00FF },      -- Magenta
    ["Bloodmoon"]  = { emoji = "🩸", color = 0xFF0000 },      -- Red
}
...
local function readWeather()
    ...
    -- Try common weather storage locations
    ...
    if weatherNode then
        ...
        -- Check for child with weather info
        ...
        -- If it's a folder, try to find the weather name inside
    ...
    -- Fallback: check for weather as direct StockValues child
    ...
    if weatherNode then
        local function checkWeatherChange()
            ...
            if currentWeather and currentWeather ~= data_weather.lastWeather then
                ...
                -- Check if this is a special weather event
                local isSpecial = SPECIAL_WEATHER[currentWeather]
                local emoji = getWeatherEmoji(currentWeather)
                
                if isSpecial then
                    -- BIG ALERT for special events
                    print("[GAG2] 🚨 SPECIAL WEATHER: " .. currentWeather .. "!")
                    
                    local rolePing = ""
                    if currentWeather == "Midas" then
                        rolePing = "<@&" .. (ROLE_IDS.legendary or "000000000000000000") .. "> "
                    elseif currentWeather == "Rainbow" then
                        rolePing = "<@&" .. (ROLE_IDS.exclusive or "000000000000000000") .. "> "
                    ...
                    sendToDiscord({
                        ...
                        embeds = {{
                            ...
                            description = "**New Weather:** " .. currentWeather,
                            color = 0x3498DB,
...
local function sendStockReport(stock)
    ...
    sendToDiscord({
        ...
        embeds   = {{
            ...
            footer    = { text = "v5 Fixed • Direct StockValues read • Weather tracking • " .. os.date("%H:%M:%S") },
            timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ"),
        }}
    })
end
...
-- MAIN
-- =====================
print("[GAG2] v5 FIXED started - Better pet detection")

sendToDiscord({
    username = "GAG2 Stock Tracker",
    embeds   = {{
        title       = "✅ GAG2 Stock Tracker v5 FIXED Online",
        description = "Reading from **ReplicatedStorage.StockValues**\n\nTracking:\n🌱 Seeds  ⚙️ Gears  📦 Crates  ⭐ Exclusive\n👑 Legendary & Super predictions only\n🐾 Pet spawn notifier with improved detection\n🌤️ Weather system with real-time alerts",
        color       = 0x5865F2,
...
while true do
    ...
    end    ["Ghost Pepper"] = true, ["Poison Ivy"] = true, ["Baby Cactus"] = true,
    ["Glow Mushroom"] = true, ["Romanesco"] = true, ["Horned Melon"] = true,
    ["Gold"] = true, ["Rainbow"] = true, ["Dragon Fruit"] = true,
    ["Dragon's Breath"] = true, ["Venus Fly Trap"] = true,
    ["Sunflower"] = true, ["Coconut"] = true
